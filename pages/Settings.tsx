...
const Settings: React.FC<{ onLanguageChange: (lang: 'fr' | 'en') => void, t: any }> = ({ onLanguageChange, t }) => {
  const navigate = useNavigate();
  const [api, setApi] = useState<APIConfig>(() => {
    const saved = localStorage.getItem('seomate_api');
    return saved ? JSON.parse(saved) : { 
      defaultModel: AIModelType.PRO, 
      autoKeywords: true, 
      devMode: true,
      siteLanguage: 'fr'
    };
  ...
  const handleLangToggle = (newLang: 'fr' | 'en') => {
    const newApi = { ...api, siteLanguage: newLang };
    setApi(newApi);
    localStorage.setItem('seomate_api', JSON.stringify(newApi));
  ...
  return (
    <div className="space-y-12 pb-24 animate-in fade-in duration-700">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="text-4xl font-black text-slate-900 tracking-tighter">{t.set_title}</h1>
          <p className="text-slate-500 font-medium mt-2">{t.set_subtitle}</p>
        </div>
        <button 
        ...
        >
          ...
          {saveStatus === 'saved' ? t.set_saved : t.set_save}
        </button>
      </header>
        ...
        <div className="lg:col-span-8 space-y-10">
          <section className="bg-white p-10 rounded-[48px] border border-slate-100 shadow-2xl space-y-8">
            <div className="flex items-center gap-4 mb-2">
              ...
              <div>
                <h3 className="text-xl font-black text-slate-900">{t.set_lang_title}</h3>
                <p className="text-sm text-slate-400 font-bold">{t.set_lang_subtitle}</p>
              </div>
            </div>
            
            <div className="flex items-center justify-between py-6 gap-4">
              <div className="max-w-md">
                <p className="font-black text-slate-900 text-base">{t.set_lang_site}</p>
                <p className="text-xs text-slate-400 font-bold mt-1">French / English UI switch.</p>
              </div>
              <div className="flex p-1.5 bg-slate-50 rounded-[24px]">
                <button 
                  onClick={() => handleLangToggle('fr')}
                  className={`px-6 py-2.5 rounded-[20px] text-[10px] font-black uppercase tracking-widest transition-all ${api.siteLanguage === 'fr' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400'}`}
                >
                  Français
                </button>
                <button 
                  onClick={() => handleLangToggle('en')}
                  className={`px-6 py-2.5 rounded-[20px] text-[10px] font-black uppercase tracking-widest transition-all ${api.siteLanguage === 'en' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-400'}`}
                >
                  English
                </button>
              </div>
          ...
          <section className="bg-white p-10 rounded-[48px] border border-slate-100 shadow-2xl space-y-8">
            <div className="flex items-center gap-4 mb-2">
              ...
              <div>
                <h3 className="text-xl font-black text-slate-900">{t.set_ai_title}</h3>
                <p className="text-sm text-slate-400 font-bold">{t.set_ai_subtitle}</p>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between py-6 border-b border-slate-50 gap-4">
                <div className="max-w-md">
                  <p className="font-black text-slate-900 text-base">{t.set_ai_engine}</p>
                  <p className="text-xs text-slate-400 font-bold mt-1">AI core selection.</p>
                </div>
                <select 
              ...
              <div className="flex items-center justify-between py-6 border-b border-slate-50 gap-4">
                <div className="max-w-md">
                  <p className="font-black text-slate-900 text-base">{t.set_ai_extract}</p>
                  <p className="text-xs text-slate-400 font-bold mt-1">Smart semantic extraction.</p>
                </div>
                <div 
              ...
              <div className="flex items-center justify-between py-6 gap-4">
                <div className="max-w-md">
                  <p className="font-black text-slate-900 text-base flex items-center gap-2">{t.set_ai_dev} <ShieldCheck className="w-4 h-4 text-green-500" /></p>
                  <p className="text-xs text-slate-400 font-bold mt-1">Dev console logging.</p>
                </div>
                <div 
        ...
        <div className="lg:col-span-4 space-y-8">
           <div className="bg-indigo-600 rounded-[48px] p-8 text-white relative overflow-hidden shadow-3xl">
              <div className="relative z-10">
                ...
                <div className="flex items-baseline gap-2 mb-6">
                   ...
                   <span className="text-sm font-black uppercase opacity-60 tracking-widest">words</span>
                </div>
                <div className="space-y-3 mb-10">
                   <div className="w-full h-2 bg-white/20 rounded-full overflow-hidden">
                      <div className="h-full bg-white w-[65%] shadow-[0_0_15px_white]"></div>
                   </div>
                   <p className="text-[9px] font-black uppercase tracking-widest text-center opacity-60">Usage: 65%</p>
                </div>
                <button 
                  onClick={() => navigate('/billing')}
                  className="w-full py-5 bg-white text-indigo-600 rounded-[24px] font-black text-[10px] uppercase tracking-[0.3em] hover:bg-slate-900 hover:text-white transition-all active:scale-95 shadow-2xl"
                >
                   {t.set_token_recharge}
                </button>
              </div>
           ...
           <div className="bg-white p-10 rounded-[48px] border border-slate-100 shadow-2xl">
              <h3 className="text-xs font-black uppercase tracking-widest text-slate-400 mb-8 flex items-center gap-2">
                 <Smartphone className="w-4 h-4" /> {t.set_sec_mobile}
              </h3>
              <div className="space-y-4">
                 ...
                 >
                    <div>
                      <p className="text-sm font-black text-slate-700">{t.set_sec_logs}</p>
                      <p className="text-[9px] font-bold text-slate-400">View IP</p>
                    </div>
                 ...
                 >
                    <div>
                      <p className="text-sm font-black text-slate-700">{t.set_sec_devices}</p>
                      <p className="text-[9px] font-bold text-slate-400">Manage sites</p>
                    </div>
                    <ArrowRight className="w-4 h-4 text-slate-300 group-hover:translate-x-1 transition-transform" />
                 </button>
              </div>
      ...
      {activeModal === 'logs' && (
        <Modal title={t.set_sec_logs} onClose={() => setActiveModal('none')}>
          <div className="space-y-4">
            {MOCK_LOGS.map(log => (
      ...
      {activeModal === 'devices' && (
        <Modal title={t.set_sec_devices} onClose={() => setActiveModal('none')}>
          <div className="space-y-4">
