...
const Dashboard: React.FC<DashboardProps> = ({ subscription, articles }) => {
      ...
      <header className="flex flex-col xl:flex-row xl:items-center justify-between gap-10">
        <div>
          <h1 className="text-5xl md:text-7xl font-black text-slate-900 tracking-tighter">
            Bonjour, <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-slate-900 italic">{subscription.firstName || 'Expert'}</span>
          </h1>
          <p className="text-slate-400 font-semibold text-2xl mt-6 opacity-70">Console de Performance SEO • Plan {subscription.planName}</p>
        </div>
        <button onClick={() => navigate('/generate')} className="flex items-center gap-4 bg-slate-900 text-white px-10 py-5 rounded-[32px] font-black text-sm uppercase tracking-[0.2em] shadow-2xl hover:bg-indigo-600 transition-all group active:scale-95">
          <Sparkles className="w-5 h-5 group-hover:rotate-12" /> NOUVELLE GÉNÉRATION
        </button>
      </header>

      {/* Vue d'ensemble */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
        <StatCard title="Articles Produits" value={articles.length} icon={FileText} trend={12} colorClass="bg-slate-900" />
        <StatCard title="Score SEO Moyen" value={`${avgSeoScore}%`} icon={Award} trend={5} colorClass="bg-indigo-600" />
        <StatCard title="Mots Générés" value={`${articles.length * 1200}`} icon={Activity} trend={8} colorClass="bg-purple-600" />
        <StatCard title="Quota Restant" value={subscription.articlesLimit === Infinity ? '∞' : Math.max(0, subscription.articlesLimit - subscription.usedArticles)} icon={Zap} trend={2} colorClass="bg-amber-500" />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        {/* Barre de santé / usage */}
        <div className="lg:col-span-2 bg-slate-950 p-12 rounded-[64px] text-white shadow-4xl relative overflow-hidden group">
          ...
          <div className="relative z-10 space-y-12">
            <div className="flex justify-between items-start">
              <div>
                <h3 className="text-[11px] font-black uppercase tracking-[0.5em] opacity-30 mb-4">Analyse de la Production</h3>
                <div className="flex items-baseline gap-4">
                  <p className="text-8xl font-black tracking-tighter">{Math.min(usagePercent, 100).toFixed(0)}%</p>
                  <span className="text-indigo-500 font-black text-xl uppercase italic">Utilisé</span>
                </div>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-between text-[10px] font-black uppercase tracking-widest opacity-40">
                ...
                <span>Optimisation Maximale</span>
              </div>
            </div>
            
            <p className="text-base font-medium opacity-50 leading-relaxed max-w-lg">
               Votre stratégie de contenu est {usagePercent < 50 ? 'en phase de démarrage' : 'en pleine expansion'}. 
               {usagePercent >= 90 ? " Vous approchez de votre limite mensuelle." : ` Vous avez encore de la marge pour ${subscription.articlesLimit === Infinity ? 'une infinité d' : (subscription.articlesLimit - subscription.usedArticles)} articles.`}
            </p>
          </div>
        </div>
        ...
        <div className="glass p-10 rounded-[64px] shadow-3xl border border-white overflow-hidden flex flex-col">
          <h3 className="text-2xl font-black text-slate-900 flex items-center gap-3 mb-10">
            <History className="w-6 h-6 text-indigo-600" /> Flux Recent
          </h3>
          <div className="space-y-4 flex-1 overflow-y-auto pr-2">
             ...
             {articles.length === 0 && (
               <div className="h-full flex flex-col items-center justify-center text-center py-10 opacity-30">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-widest">Aucun article généré</p>
               </div>
             )}
          </div>
